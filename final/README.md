...
Серверам TCP-PROD, FTP-PROD информационной системы PROD1 нужно обеспечить связность с PROD3.
Серверу HTTPS-PROD информационной системы PROD2 нужно обеспечить выход в интернет и видимость из интернета через статический NAT.
Также все маршрутизаторы и серверы должны иметь доступ к NTP-серверу (в VLAN SERVERS) и должны быть доступны из Management сети (VLAN Management) для контроля и управления.
