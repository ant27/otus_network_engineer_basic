...
```
S1(config)#switchport port-security violation {shutdown | restrict | protect}
```
...
- protect: Это наименее безопасный из режимов нарушения безопасности. Порт отбрасывает пакеты с неизвестными адресами источника, пока вы не удалите достаточное количество безопасных MAC-адресов, чтобы опуститься ниже максимального значения или увеличить максимальное значение. Сообщения в системный жкрнал не записываются.
